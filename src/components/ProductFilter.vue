        ...
        <ul class="colors">
          <li class="colors__item" v-for="(color, index) in colors" :key="color.id">
            <label class="colors__label">
              ...
              type="radio" name="color" :value="color.id" v-model="currentColorId">

              <span class="colors__value" :style="style(index)">
              </span>
            </label>
          </li>
        </ul>
...
export default {
  ...
  methods: {
    ...
    reset() {
      ...
      this.$emit('update:priceTo', 150000);
      this.$emit('update:categoryId', 0);
      this.$emit('update:colorId', 0);
      this.$emit('resetPage', 1);
    },
    style(i) {
      return `backgroundColor: ${this.colors[i].code}`;
    },
  },
};
</script>
